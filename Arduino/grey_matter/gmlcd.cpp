...
// MIT license
// I ripped off this code for Grey Matter - Jonathan Forhan
...
GMLCD::GMLCD(uint8_t cs, uint8_t cd, uint8_t wr, uint8_t rd, uint8_t reset)
...
//GMLCD::GMLCD(void) : Adafruit_GFX(TFTWIDTH, TFTHEIGHT)
//{
//    init();
//}

// Initialization common to both shield & breakout configs
void GMLCD::init(void)
{
...
void GMLCD::setAddrWindow(int x1, int y1, int x2, int y2)
{
    CS_ACTIVE;
    if (driver == ID_932X)
    {
...
void GMLCD::flood(uint16_t color, uint32_t len)
{
    uint16_t blocks;
...
void GMLCD::drawFastHLine(int16_t x, int16_t y, int16_t length, uint16_t color)
{
    int16_t x2;
...
void GMLCD::drawFastVLine(int16_t x, int16_t y, int16_t length, uint16_t color)
{
    int16_t y2;
...
void GMLCD::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h, uint16_t fillcolor)
{
    int16_t x2, y2;
...
void GMLCD::fillScreen(uint16_t color)
{
...
void GMLCD::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void GMLCD::pushColors(uint16_t *data, uint8_t len, boolean first)
{
    uint16_t color;
    uint8_t hi, lo;
    CS_ACTIVE;
...
void GMLCD::setRotation(uint8_t x)
{
...
uint16_t GMLCD::readPixel(int16_t x, int16_t y)
{
...
uint32_t GMLCD::readReg(uint8_t r)
{
    uint32_t id;
    uint8_t x;
...
uint16_t GMLCD::color565(uint8_t r, uint8_t g, uint8_t b)
{
...
#ifndef write8
void GMLCD::write8(uint8_t value)
{
    write8inline(value);
}
#endif
...
void GMLCD::writeRegister8(uint8_t a, uint8_t d)
{
...
void GMLCD::writeRegister16(uint16_t a, uint16_t d)
{
...
void GMLCD::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d)
{
    writeRegisterPairInline(aH, aL, d);
}
#endif

void GMLCD::writeRegister24(uint8_t r, uint32_t d)
{
    CS_ACTIVE;
    CD_COMMAND;
    write8(r);
...
void GMLCD::writeRegister32(uint8_t r, uint32_t d)
{
    CS_ACTIVE;
    CD_COMMAND;
    write8(r);
